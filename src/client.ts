...
import cloudperson from './img/cloudplayer-Sheet.png'

import enemy from './img/enemy-Sheet.png'
...
class GameScene extends Scene {
    ...
    private enemy_sprites: { [key: string]: Sprite }
    ...
    constructor(engine: ToastEngine) {
        ...
        this.cloud_sprites = {
            "idle": new Sprite(engine, cloudperson, { width: 32, height: 32})
        }
        ...
        this.enemy_sprites = {
            "idle": new Sprite(engine, enemy, { width: 32, height: 32 })
        }
        this.enemies = []
    ...
    public update(dt: number) {
        ...
        this.cloud.position.x = this.player.position.x
        this.cloud.position.y = this.player.position.y - 70
        this.cloud.frame.x = (this.playerFlip ? 3 : 0) + (this.cloud.frame.x + 4 * dt) % 3

        this.enemies.forEach(enemy => {
            enemy.frame.x = (enemy.frame.x + 4 * dt) % 4
        })
